...
export class iceCache {
    ...
    public static getFromSession<T>(item: string): T {
        return iceCache.getVariable(item);
    }
    public static setFromSession<T>(item: string,valeur:T) {
        return iceCache.setVariable(item,valeur);
    }
    ...
    public static genericCache<T>(/* nom unique à donner pour pouvoir stocker les données du cache*/nomVar: string,
        ...
        let myThis: typeof iceCache = this;
    ...
    public static getDicoWithDelete<T>(paramsDico: iCacheGeneric<T>, page: any): { data: Promise<T>, reset: () => void } {
        ...
        return {
            data: iceCache.genericCache<T>(paramsDico.nomVariableUnique,
                (page) => {
        ...
        , reset: () => {
            ...
            iceCache.deleteVariable(paramsDico.nomVariableUnique);
        }
    };
    }
    ...
    public static get dicoRessources(): Dictionnaire<string> {
        ...
        if (iceCache._dicoRessource == undefined) {
            iceCache._dicoRessource = iceCache.getVariable("dicoRessources");
            //si la variable top n'existe pas '

            //on va le chercher sur le serveur
            if (iceCache._dicoRessource == undefined) {//on essaye de recuperer le dico sur le top
                ...
                if (xClass.config.langDictionaryData != undefined) {
                    iceCache._dicoRessource = xClass.config.langDictionaryData;
                }
                else {
                    iceCache._dicoRessource = xRequire(iceOutils.convertDevUrlToRelativeUrl(xClass.config.langDictionaryUrl));
                } 

                //et on essaye de le mettre ds le top pour gagner du temps la prochaine fois
                iceCache.setVariable("dicoRessources", iceCache._dicoRessource);
                if (iceCache._dicoRessource == undefined) { alert('impossible de charger le dictionnaire'); }
    ...
    public static get debugMode(): boolean {
        if (iceCache._debugMode == undefined) {
            //            iceCache._debugMode = iceCache.getVariable("debugMode");
            iceCache._debugMode = xClass.localConfig.debug;
            if (iceCache._debugMode == undefined) {
                iceCache._debugMode = false;
            }
        }
        return iceCache._debugMode;
    }
    ...
    public static setTraductionManquante(val: string,commentaire :string) {
        ...
        if (false /*xClass.debugMode*/) {
            ...
            iceOutils.afficherMessageConfirmation(message, true,
                (choix) =>
                {
                    if (!isOverLength)
                    {
                        ...
                        {
                            iceOutils.afficherMessageAlertifyLocaliseLog('traduction à remplacer par un textVariable');
                        }
                    }
                    else
                    {
                        iceOutils.afficherMessageAlertifyLocaliseLog('traduction à remplacer par un code plus court');
                    }
