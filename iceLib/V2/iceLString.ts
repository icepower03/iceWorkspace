...
import { iceCache } from './iceCache';

/**
 * Classe pour gérer la localisation des strings
 */
export class iceLString {
    ...
    constructor(inVal: string) {
        ...
        if (inVal.length > 0 && btrimstart) // Si la chaine n'est pas vide et qui il y des vrais caracteres
        {
            ...
            if (inVal.length > 200)
            {
                ...
                iceCache.setTraductionManquante(inVal, logpile);
                myThis._text = inVal;
            }
            else
            {
                ...
                let retour: string;
                retour = iceCache.dicoRessources[myThis.code];
                ...
                else
                {
                    if (retour === undefined) 
                    {
                        ...
                        (iceCache.dicoRessources as any)[myThis.code] = null;
                        myThis._text = trimstart + myThis.code + trimend;
                        try
                        {
                            let error: Error = new Error();
                        ...
                        iceCache.setTraductionManquante(myThis.code, logpile);
                    }
                    else 
                    {
