...
export class xxPlanning implements iXElement {
    ...
    private RenderRdv: (rdv: PlanningRdv) => xxGrid | xxToolTip | xxLabel | xxWrapPanel | xxStackPanel | xDiv;
    ...
    public idTimerDivCurseur: number;
    ...
    private _PrevisualisationRdv: xxGridItem = null;
    private get PrevisualisationRdv(): xxGridItem { return this._PrevisualisationRdv; };
    private set PrevisualisationRdv(intt: xxGridItem) { this._PrevisualisationRdv = intt; };
    ...
    public contenuMenuClicDroit: xDiv;

    public get y() {
        let myThis: xxPlanning = this;
            ...
            if (myThis.AddRdvOnClick.PrevisualisationSouris != null) {
                ...
                if (myThis.AddRdvOnClick.PrevisualisationSouris.RenderPrevisualisationSouris == null)
                    myThis.AddRdvOnClick.PrevisualisationSouris.RenderPrevisualisationSouris = (ici, rdv) => {
                        ici.append(new xxLabel({ textVariable: DateSerialisable.tolocalstringHeureMinute(rdv.DateDebut) + " - " + DateSerialisable.tolocalstringHeureMinute(rdv.DateFin) }))
                }
            }
        }
    ...
    private createColonneZoomEtHeure() {
        ...
        let gridZoomEtParam = new xxGrid({
            colonnes: ["auto"],
            lignes: ["auto", "auto"],
            gridGap: "3px",
            fullHeight: true,
        ...
        //let stackPanelZoomBtnParam = new xxStackPanel({ espaceMinimaliste: true });

        myThis.gridZoomEtHeure = new xxGrid({
            colonnes: ['1'],
            lignes: listeLignes,
            gridGap: "0px",
            class: 'ZoomEtHeure'
        ...
        myThis.gridZoomEtHeure.append([new xxGridItem({
            class: "premierItem",
            colStart: 1,
            rowStart: 1,
            nbCols: 1,
            nbRows: 1,
            cssOnly: true,
            content: myThis.zoom || myThis.AddRdvOnClick != null ? gridZoomEtParam : new xDiv({ class: "zoomItem" })
        })]);

        if (myThis.zoom && myThis.AddRdvOnClick == null) {
            let zoomItem = new xxZoom({
            ...
            gridZoomEtParam.append([new xxGridItem({ colStart: 1, rowStart: 1, nbRows: 2, content: zoomItem, optionsAffichage: { alignementContenu: enumAlignementContenu.CentreCentre } })])
            //stackPanelZoomBtnParam.append(zoomItem);
        }else if (myThis.AddRdvOnClick != null) { 
            
            let pageBoxer = new xxPageWrapper({ titleLocalise: "Paramètrage du planning" });
            let boxerParametrage = new xxBoxer({ initContent: pageBoxer, tailleBoxer: enumBoxerTaille.fit, class:"BoxerParametrage" });

            let stackBoxerParam = new xxStackPanel({ espaceMinimaliste: true, gap: 10 });
            ...
            if (myThis.zoom) {
                ...
                gridZoomEtParam.append([new xxGridItem({ colStart: 1, rowStart: 1, nbRows: 1, content: zoomItem, optionsAffichage: { alignementContenu: enumAlignementContenu.CentreCentre } })]);

                stackBoxerParam.append(new xxLabelContainer({
                    textLocalise: "Zoom",
            ...
            gridZoomEtParam.append([new xxGridItem({
            ...
            if (myThis.AddRdvOnClick != null) {
                let listeArrondissement: xxListeDeroulante<number> = new xxListeDeroulante({
                ...
                let boutonMoins: xxBouton = new xxBouton({
                    ...
                    click: cb => {
                        if (myThis.listArrondissementPossible.indexOf(myThis.AddRdvOnClick.ArrondirDebutRdvAuPas) - 1 >= 0)
                            ...
                            xOutils.afficherMessageAlertifyError("Impossible d'avoir une tranche horraire plus courte");
                        cb();
                    }
                });
                ...
                let boutonPlus: xxBouton = new xxBouton({
                    ...
                    click: cb => {
                        if (myThis.listArrondissementPossible.indexOf(myThis.AddRdvOnClick.ArrondirDebutRdvAuPas) + 1 <= myThis.listArrondissementPossible.length - 1)
                            ...
                            xOutils.afficherMessageAlertifyError("Impossible d'avoir une tranche horraire plus longue");
                        cb();
                    }
                });
                ...
                stackBoxerParam.append(new xxLabelContainer({
                    ...
                    initContent: new xxWrapPanel({ class:"wpTrancheHoraire", espaceMinimaliste: true, initContent: [listeArrondissement, boutonMoins, boutonPlus] })
                }));
        ...
        for (let i = 0; i <= lignes; i++) {
            ...
            if (calculHeures != 24)
                Heures.push(new xTime(calculHeures, calculMinutes).getString());
            else
                Heures.push(new xTime(0, calculMinutes).getString());
        ...
        for (let i = 1; i <= (lignes * 12); i += 12) { //myThis.celluleLength
            let libelle = new xDiv({ class: "libellePlanning" });

            //(car la liste commence à 0 et que la première ligne est occupée par les libellés des colonnes !)            

            libelle.asHolder.append(new xSpan({ class: "heure", textVariable: Heures[j] }));
            let div = new xxGridItem({
    ...
    public masquerAllRdv(): void {
        ...
        myThis.getAllRdvs().forEach(item => {
            cacherxElements(item.Item, true);
        });
    }
    ...
    private afficherPrevisualisation(indiceColonne: number, rowStart: number, nbRows: number, colStart: number, fromDragAndDrop: boolean) {
        ...
        if (colonne != null) {
            ...
            if (fromDragAndDrop)
                content = myThis.RenderDivCurseur != undefined ? myThis.RenderDivCurseur(myThis.RdvSelect) : new xDiv({ class: "RenderDivCurseur" })
            else {
                content = new xDiv({});
                myThis.AddRdvOnClick.PrevisualisationSouris.RenderPrevisualisationSouris((<xDiv>content).asHolder, myThis.rdvPrevisualiseFictif);
            }


            myThis.PrevisualisationRdv = new xxGridItem({
                rowStart: rowStart,
                nbRows: nbRows,
    ...
    private LoadHeaderGridMonth() {
        ...
        myThis.GridMonth = new xxGrid({
            ...
            .append([new xxGridItem({
                colStart: 1,
                rowStart: 1,
                nbCols: 1,
                nbRows: 1,
                content: new xxLabel({ textLocalise: "Plage", type: enumTypeLabel.important, centrer: true, }),
                class: "premiereLigne",
            })])
            .append([new xxGridItem({
                colStart: 2,
                ...
                nbCols: 1,
                nbRows: 1,
                content: new xxLabel({ textLocalise: "Lundi", type: enumTypeLabel.important }),
                class: "premiereLigne",
            })])
            .append([new xxGridItem({
                colStart: 3,
                ...
                nbCols: 1,
                nbRows: 1,
                content: new xxLabel({ textLocalise: "Mardi", type: enumTypeLabel.important }),
                class: "premiereLigne",
            })])
            .append([new xxGridItem({
                colStart: 4,
                ...
                nbCols: 1,
                nbRows: 1,
                content: new xxLabel({ textLocalise: "Mercredi", type: enumTypeLabel.important }),
                class: "premiereLigne",
            })])
            .append([new xxGridItem({
                colStart: 5,
                ...
                nbCols: 1,
                nbRows: 1,
                content: new xxLabel({ textLocalise: "Jeudi", type: enumTypeLabel.important }),
                class: "premiereLigne",
            })])
            .append([new xxGridItem({
                colStart: 6,
                ...
                nbCols: 1,
                nbRows: 1,
                content: new xxLabel({ textLocalise: "Vendredi", type: enumTypeLabel.important }),
                class: "premiereLigne",
            })])
            .append([new xxGridItem({
                colStart: 7,
                ...
                nbCols: 1,
                nbRows: 1,
                content: new xxLabel({ textLocalise: "Samedi", type: enumTypeLabel.important }),
                class: "premiereLigne",
            })])
            .append([new xxGridItem({
                colStart: 8,
                ...
                nbCols: 1,
                nbRows: 1,
                content: new xxLabel({ textLocalise: "Dimanche", type: enumTypeLabel.important }),
                class: "premiereLigne",
            })])
    ...
    private LoadContenuGridMonth(rdvs: PlanningRdv[]) {
        ...
        for (let semaine = 0; semaine < nbLignes; semaine++) {
            ...
            let label = new xxLabelContainer({ optionsAffichage: { positionDuContenu: enumPositionDuContenu.bas } });

            label.append(new xxLabel({ textVariable: new xLString("Semaine n°").text + numWeek, type: enumTypeLabel.important }));
            label.asHolder.append(new xxLabel({ textVariable: "Du " + DateSerialisable.tolocalStringOnlyDate(jourCourant) })),
                label.asHolder.append(new xxLabel({ textVariable: "au " + DateSerialisable.tolocalStringOnlyDate(DateSerialisable.addDaysSerialisable(jourCourant, 6)) }))
    ...
    public setModeAffichage(typeAffichage: EPlanningTypeAffichage, rdvs: PlanningRdv[], dispos: PlanningDisponibilite[], nbJoursAffiches: number, isTypeIntervaleSemaine: boolean) {
        ...
        if (myThis.contenuePrincipal == null) {
            myThis.contenuePrincipal = new xxWrapPanel({
        ...
        if (myThis.typeAffichage == EPlanningTypeAffichage.Standard) {
            myThis.contenuMenuClicDroit = new xDiv({ class: "contextPlanning" });
            ...
            myThis.callbackToDeleteRightClick = () => {
                myThis.contenuMenuClicDroit.y.style.zIndex = "0";
                cacherxElements(myThis.contenuMenuClicDroit, true);
            }
    ...
    private LoadParamUser() {
        ...
        myThis.ListParamUser = JSON.parse(xOutils.getLocalStorage(KeyLocalStorageParamUser));
    ...
    private SaveParamUser() {
        ...
        if (myThis.keyPlanning != null) {
            ...
            xOutils.setLocalStorage(KeyLocalStorageParamUser, JSON.stringify(myThis.ListParamUser));
        }
    }
