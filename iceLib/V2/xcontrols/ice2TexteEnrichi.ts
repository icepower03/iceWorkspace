...
export class ice2TexteEnrichi implements iXElement{
    ...
    {
        let myThis: ice2TexteEnrichi = this;
        myThis.elementPrincipal = new iceDiv({
            class: "ice2TexteEnrichi " + (options.class != undefined) ? options.class : '',
        });
        ...
        if (options.id != null)
            ...
            myThis.textArea.y.id = "id_ice2TexteEnrichi"; 
        ...
        else
        {
            gestion_editor.on('instanceReady', function (evt: any)
            {
                ...
                if (menu_list_mots == undefined)
                {
                    ...
                    menu_list_mots.innerHTML = new iceLString('Chargement en cours...').text;
