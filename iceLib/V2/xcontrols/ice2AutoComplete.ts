...
import { xInputText, enumStyleInput } from './iceInput';
...
export class ice2AutoComplete<T> implements iXElement
{

    private listDeroulante: ice2ListeDeroulante<ShownItem<T>>;
    private elementAutocomplete: iXElement;

    private binding: BindableObject<T>;
    private input: iceInputTextAvecIcone;
    private boutonMore: ice2Bouton;
    ...
    public constructor(options: OptionsAutoComplete<T>)
    {
        ...
        let classe: string = "ice2AutoComplete" + (options.class != undefined ? " " + options.class : "");
                ...
                options.binding.visibility.bind(s =>
                {
                    switch (s)
                    {
                        case enumVisibility.afficher:
                            affichericeElements(mythis.listDeroulante);
                            break;
                        case enumVisibility.masquer:
                            cachericeElements(mythis.listDeroulante, false);
                            break;
                        case enumVisibility.masquerAvecCollapse:
                            cachericeElements(mythis.listDeroulante, true);
                            break;
                    }
                });
            }
        ...
        mythis.listDeroulante = new ice2ListeDeroulante<ShownItem<T>>({
            donnees: [],
            class: classe,
            nonResponsive: true,
            id: options.id,
            ...
            renderSelected: (place, item, open) =>
            {
                ...
                {
                    mythis.input = new iceInputTextAvecIcone({
                        ...
                        id: 'ice2AutoComplete_input',
                        class: 'ice2AutoComplete_input' + (options.taille != null ? ' taille_' + options.taille : ""),
                        autoChange: true,
            ...
            renderSelectItem: (p, item, select, idElemnt) =>
            {
                if (mythis.renderItemInListe == null)
                {
                    ...
                    if (item.value != null)
                    {
                        ...
                        buttonTemp = new ice2Bouton({
                            id: idElemnt != null && idElemnt != "" ? "ice2AutoComplete_bouton_" + idElemnt : null,
                            textVariable: item.cle,
                            titleLocalise:"",
                            class: "ice2AutoComplete_bouton " + mythis.getClass(item.value, mythis.allDatas),
                            optionsAffichage: {
                    ...
                    else
                    {
                        buttonTemp = new ice2Bouton({
                            ...
                            id: idElemnt != null && idElemnt != "" ? "ice2AutoComplete_bouton_" + idElemnt : null,
                            class: 'ice2AutoComplete_bouton bouton_null',
            ...
            {
                placeHead.append(this.input = new iceInputTextAvecIcone({
            ...
            renderEndList: (ici) =>
            {
                mythis.boutonMore = new ice2Bouton({
                ...
                if (mythis.datasFiltre != null && mythis.datasFiltre.length < mythis.NB_AFFICHAGE_LISTE)
                    cachericeElements(mythis.boutonMore, true);
            },
            selected: (item) =>
            {
        ...
        if (mythis.optionsAffichage != undefined)
        {
            ...
            iceStyle.AppliquerOptionsAffichage(mythis.listDeroulante, mythis.optionsAffichage);
        }


        mythis.datas = [];
        mythis.value = options.typeValue != undefined ? mythis.getLibelle(options.typeValue) : options.value;
        mythis.labelLectureSeule = new ice2Label({
            textLocalise: mythis.value != null ? mythis.value : "",
            id: options.id,
            class: classe
        });
        mythis.holderLectureSeule = new iceDiv({
            id: options.id,
            class: classe
        }).asHolder;
    ...
    private filtrerListe(newFiltre: string)
    {
        ...
        if (newFiltre != mythis.filtre || newFiltre == "")
        {
            ...
            affichericeElements(mythis.boutonMore);
    ...
    public openList(): ice2AutoComplete<T>
    {
        this.listDeroulante.ouvrirSelection();
        return this;
    }

    public closeList(): ice2AutoComplete<T>
    {
        this.listDeroulante.close();
        return this;
    }

    public select(item: T): ice2AutoComplete<T>
    {
