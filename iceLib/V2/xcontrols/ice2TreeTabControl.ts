...
interface OptionsTreeTabControl {
    ...
    addContentTreeTabControl?: ice2TreeTabControl;
    postZoneAligneeADroite?: boolean;

}

export class ice2TreeTabControl implements iXElement  {
   
    public get y() { return this.TabControlPrincipal.y; }
    private favoriteGlobalKey?: string;
    private TabControlPrincipal: ice2TabControl;
    private modeNavigation?: boolean;
    
    public appendPreZoneTab(i: iXElement): ice2TreeTabControl {
        let myThis: ice2TreeTabControl = this;
        myThis.TabControlPrincipal.appendPreZoneTab(i);
        return myThis;
    }

    public appendPostZoneTab(i: iXElement): ice2TreeTabControl {
        let myThis: ice2TreeTabControl = this;
    ...
    public ajouterGroupeOnglets(op: OptionsTreeTabControl) :ice2TreeTabControl {
        let myThis: ice2TreeTabControl = this;
        ...
        myThis.TabControlPrincipal.ajouterOnglet({
            ...
            addContent: function (ici: xElementHolder) {
                ...
                if (newOptions.addContentTreeTabControl != undefined) {
                    ...
                    newTab = new ice2TreeTabControl(newOptions);
                ici.append(newTab);
            },
            onSelect: function () {
                // selectionner et recharger le premier sous-onglet ou le favori
                if (newOptions.addContentTreeTabControl != undefined) {
                    let tabItems: ice2TabItem[] = newOptions.addContentTreeTabControl.TabControlPrincipal.tabItems.filter(e => e.id == newOptions.addContentTreeTabControl.TabControlPrincipal.CurrentFavoriteTabKey);
    ...
    public selectTabItem(itemId: string, avecRechargement: boolean): boolean {
        let myThis: ice2TreeTabControl = this;
        let tabItems: ice2TabItem[] = myThis.TabControlPrincipal.tabItems.filter(e => e.id == itemId);
        if (tabItems.length > 0) {
    ...
    constructor(o: OptionsTreeTabControl) {
        ...
        myThis.TabControlPrincipal = new ice2TabControl({
            id: o.id,
            class: o.class,
