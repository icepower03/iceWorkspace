...
class xxColonne<T>
{
    public wrapInEntete: xxWrapPanel;
    ...
    public renderTitre?: (place: iXElementHolder, setTriPrincipal?: (colonne: xxColonne<T>, withTogle: boolean) => void, trierLesDonnes?: () => xxTableauWrapper<T>) => void;
    public renderMethod: (place: iXElementHolder, valeur: T, l: ITableauLigneWrapperBase<T>, tab: xxTableauWrapper<T>) => void;
    ...
    private static getUniqueColumn(): number {
        xxColonne.compteurUniverselColonnes++;
        return xxColonne.compteurUniverselColonnes;
    ...
    constructor(inOptions: OptionsColonnes<T>) {
        ...
        if (inOptions.titleLocalise != undefined) {
            mythis.titre = new xLString(inOptions.titleLocalise).text;
        }
        else {
        ...
        mythis._cle = xxColonne.getUniqueColumn();
    ...
    public static rechercheString(valeur1: string, valeur2: string[]): boolean {
        if (valeur1 != null && valeur1 != "") {
            ...
            return valOu.some(el => {
                let valEt = el.split('+');
                return xOutils.rechercheStringTous(valEt, valeur2);
            });
        }
...
class xxTableauLigneWrapper<T> implements ITableauLigneWrapperBase<T> {

    constructor(inItem: xxTableauItem<T>) {
        this.item = inItem;
    }

    private item: xxTableauItem<T>;
    /**
    ...
    */
    ...
    public afficherDetail(): void {
        let myThis: xxTableauLigneWrapper<T> = this;
...
export class xxTableauWrapper<T> implements iXElement {
    ...
    private footerTableau: xDiv;
    private placeInfoNoData: iXElementHolder;
    private placeInfoLoading: iXElementHolder;

    //   private placeCompteurPagination: xxLabel;
    //   private placeTotalPage: xxLabel;
    private compteurComplet: xxLabel;
    private monTableau: HTMLTableElement;
    private ligneEntete: HTMLTableRowElement;
    private mesItemsTous: xxTableauItem<T>[] = [];
    private mesItemsFiltres: xxTableauItem<T>[] = [];
    private debutAffichage: number = 0;
    private pagination: number = -1;
    private colonneTri: xxColonne<T>[] = [];
    ...
    private clickLigne: (l: T, lw: xxTableauLigneWrapper<T>) => void;
    private dblClickLigne: (l: T, lw: xxTableauLigneWrapper<T>) => void;
    public mesColonnes: xxColonne<T>[] = [];
    private dockBoutonHeader: xxDockPanelDeprecated;
    //   private _classTitle: string;
    private labelCompteur: xxLabel;
    private afficherTotalElements: boolean;
    private inputFiltre: xInputTextAvecIcone;
    ...
    private renderDetailLigne: (place: iXElementHolder, valeur: T, lw: xxTableauLigneWrapper<T>) => void;
    private printDetailLigne: (valeur: T) => string;
    private afficherDetailLigne: (valeur: T) => boolean;
    private _ligneSelectionne: xxTableauLigneWrapper<T>;


    public getColonnes(): xxColonne<T>[] {
        return this.mesColonnes;
    }

    private renderLoading() {
        let myThis: xxTableauWrapper<T> = this;
        myThis.divPrincipal.addClass('LoadingMode');
        myThis.placeInfoLoading.append(new xxLabel({ textLocalise: 'Chargement en cours', type: enumTypeLabel.information, habillage: enumHabillageLabel.loading }));
    }
    ...
    constructor(inOptions: OptionsTableau<T>) {
        ...
        if (inOptions.renderNoData == undefined) {
            ...
            myThis.renderNoData = (ici) => {
                ...
                ici.append(new xxLabel({ textLocalise: 'Aucune donnée présente dans le tableau.', type: enumTypeLabel.information }));
            }
        }
        else {
            if (inOptions.WithHeaderRenderNoData == null)
        ...
        if (inOptions.exportPDF != undefined) {
            ...
            if (inOptions.exportPDF.sousTitreLocaliseExportPdf != undefined) {
                if (typeof inOptions.exportPDF.sousTitreLocaliseExportPdf === "string")
                    myThis.sousTitrePDf = new xLString(inOptions.exportPDF.sousTitreLocaliseExportPdf).text;
                else
            ...
            if (inOptions.exportPDF.titreLocaliseExportPdf != undefined) {
                if (typeof inOptions.exportPDF.titreLocaliseExportPdf === "string")
                    myThis.titrePDF = new xLString(inOptions.exportPDF.titreLocaliseExportPdf).text;
                else
        ...
        /*
        * xxTableau est composé de 3 grandes zones : une entete / le tableau ( avec l'entête de tableau) / un footer
        *
        */
        myThis.divPrincipal = new xDiv();
        myThis.divPrincipal.addClass("xxTableauDivPrincipal");
        if (myThis.classCss)
            myThis.divPrincipal.addClass(myThis.classCss.trim());
        if (inOptions.detailLigne != undefined) {

            myThis.divPrincipal.addClass("xxTableau_avecLignesDepliables");
        }
        if (myThis.margin == true) {
            myThis.divPrincipal.addClass("AvecMarge");
        }

        let tableauHeader: Container<xxDockPanelDeprecated> = new Container<xxDockPanelDeprecated>();

        //ici j'ajoute ma zone d'entête globale

        // if (!myThis.sansTableauTools) {
        myThis.divPrincipal.asHolder.xxDockPanelDeprecated({ class: !myThis.sansTableauTools ? 'xxTableauTools' : 'xxTableauTools maskey', centrerDernier: false }, tableauHeader);
        // }

        if (myThis.titre != null) //Si le tableau a un titre
        {
            if (myThis.icone != null) { //Si le tableau a aussi une icone
                tableauHeader.content.append(new xxLabelContainer({
                    class: 'xxTableau-LC_title',
                ...
                tableauHeader.content.append(new xxLabel({ textVariable: myThis.titre, type: inOptions.titleTypeLabel }), DockPosition.gauche);
            }
        }
		...
		myThis.dockBoutonHeader = new xxDockPanelDeprecated({});
		myThis.labelCompteur = new xxLabel({ textVariable: '0' });
		if (myThis.afficherTotalElements)
		{
            tableauHeader.content.append(new xxLabelContainer({ class: 'compteurElements', textLocalise: 'élément(s)', optionsAffichage: { positionDuContenu: enumPositionDuContenu.gauche, margin: { Droite : 10 } }, initContent: myThis.labelCompteur, labelLargeurLibre:true, }), DockPosition.gauche);
		}
        ...
        if (!myThis.masquerZoneFiltreTexte) {
            ...
            myThis.inputFiltre = new xInputTextAvecIcone({
                placeHolderlocalise: myThis.placeHolderFiltreTexte,
                id:"inputFiltrexxTableau",
                class: 'saisie',
        ...
        myThis.monTableau.classList.add("xxTableau");
        ...
        myThis.ligneEntete.classList.add("xxTableauHeader");
        //ajout du tableau pour définir la deuxième zone

        myThis.divPrincipal.asHolder.y.append(myThis.monTableau);
        myThis.placeInfoNoData = new xDiv({ class: 'divCentreNoData' }).asHolder;

        myThis.placeInfoLoading = new xDiv({ class: 'divCentreDataLoading' }).asHolder;
        ...
        myThis.footerTableau = new xDiv({ class: 'footerTableau' });
        myThis.divPrincipal.asHolder.append(myThis.footerTableau);


        let infosPagination: Container<xDiv> = new Container<xDiv>();
        //       myThis.placeCompteurPagination = new xxLabel({textVariable:''});
        //       myThis.placeTotalPage = new xxLabel({ textVariable: '' });
        ...
        if (myThis.pagination > 0) {
            ...
            /*
                .xxLabelContainer({ textLocalise: 'Page', labelLargeurLibre:true, initContent: myThis.placeCompteurPagination })
                .xxLabelContainer({ textLocalise: '/', initContent: myThis.placeTotalPage });
                */
        ...
        inOptions.columns.forEach(function (c: OptionsColonnes<T>) {
            let nvlCol: xxColonne<T> = new xxColonne<T>(c);
    ...
    private initTri() {
        let myThis: xxTableauWrapper<T> = this;
        let tempTri: xxColonne<T>[] = myThis.mesColonnes
            .filter(function (c: xxColonne<T>) { return c.getOrderTri() != undefined && c.getOrderTri() > 0; })
            .sort(function (c1: xxColonne<T>, c2: xxColonne<T>) { return c1.getOrderTri() - c2.getOrderTri(); }
            )

        tempTri.forEach(
            function (col: xxColonne<T>, idx: number) {
                if (idx == 0) {
    ...
    public setFiltre(str: string): xxTableauWrapper<T> {
        let myThis: xxTableauWrapper<T> = this;
    ...
    private async genererworksheetDonneeFichierXLSX() {
        ...
        let colonnesImprimables = myThis.mesColonnes.filter(function (c: xxColonne<T>) { return c.print != undefined; })
        ...
        let itemPrecedent: xxTableauItem<T> = null;
        await myThis.mesItemsFiltres.forEach(async function (item: xxTableauItem<T>, i) {
            let uneLigne: string[] = [];
            ...
            colonnesImprimables.forEach(async (col: xxColonne<T>, i: number) => {
                if (col != undefined) {
    ...
    public exporterCSV() {
        ...
        let prepareData = (s: string) => {
            ...
            xOutils.replaceAll(s, '"', '');
            s = '"' + s + '"';
            return s;
        };


        colonnesImprimables.forEach(function (col: xxColonne<T>) { enteteColonnes.push(prepareData(col.titre)) });
        tableauDonnees.push(enteteColonnes.join(";") + String.fromCharCode(13) + String.fromCharCode(10));

        myThis.mesItemsFiltres.forEach(function (item: xxTableauItem<T>) {
            let tableauAffiche: string[] = [];

            colonnesImprimables.forEach(function (col: xxColonne<T>) { tableauAffiche.push(prepareData(col.print(item.donnee).toString())); });
    ...
    private getImagesPdfWrapper(): Promise<dicoImagesXElement> {
        ...
        if (myThis.getImagesPdf != undefined) {
            ...
            if (xElementsFramework.getImages != undefined) {
                return xElementsFramework.getImages();
            }
            else {
     ...
     * @param o
     */
    public getLigneByValue(o: T): xxTableauLigneWrapper<T> {
        let myThis: xxTableauWrapper<T> = this;
        let retour: xxTableauItem<T>;
        myThis.mesItemsTous.forEach(
            function (i: xxTableauItem<T>) {
                if (o == i.donnee) { retour = i; }
            }
        )

        return new xxTableauLigneWrapper<T>(retour);

    }

    public async exporterPDF(cb: () => void) {

        await xOutils.inclureLibrairie(EnumLibrairieJs.pdfMake);
            ...
            footer: function (currentPage, pageCount) {
                return {
                    ...
                    columns: [
                        ...
                        { text: new xLString("Edité le {0}").format([DateSerialisable.ToStringDateHeure(DateSerialisable.Now())]), width: (widthpage / 2) + marginAjuste } //xOutils.DateToFrenchDateString(new Date(), true, false)])
                    ]
                };
            },
            header: [],
        ...
        let colonnesImprimables = myThis.mesColonnes.filter(function (c: xxColonne<T>) { return c.print != undefined; })
        colonnesImprimables.forEach(function (col: xxColonne<T>) {
     ...
     */
    public ajouterBouton(b: xxToolTipBouton | xxBouton, position: DockPosition): xxTableauWrapper<T> {
        let myThis: xxTableauWrapper<T> = this;
   ...
   * @param position Dockposition
   */
    public ajouterLabelContainer(b: xxLabelContainer, position: DockPosition): xxTableauWrapper<T> {
        let myThis: xxTableauWrapper<T> = this;
        myThis.dockBoutonHeader.append(b, position);
        return myThis;
    }
     ...
     * @param position
     */
    public ajouterElement(b: iXElement, position: DockPosition): xxTableauWrapper<T> {
        let myThis: xxTableauWrapper<T> = this;
        myThis.dockBoutonHeader.append(b, position);
        return myThis;
    }
    ...
    private addDatas(tabVal: T[]): xxTableauWrapper<T> {
        let myThis: xxTableauWrapper<T> = this;

        tabVal.forEach(function (val: T, index: number) {
            let ligne: HTMLTableRowElement;
            let item: xxTableauItem<T>;

            ligne = document.createElement("tr");
            ligne.classList.add("xxTableauItem");

            let renderPlaces: Dictionnaire<HTMLTableDataCellElement> = {};

            item = new xxTableauItem<T>(ligne, renderPlaces, val, myThis);
            ...
            if (myThis.clickLigne != undefined) {
                ligne.addEventListener('click', (ev: MouseEvent) => {
                    ...
                    let lw = new xxTableauLigneWrapper<T>(item);
                    lw.toggleClass("ligneSelectionnee");
                    myThis._ligneSelectionne = lw;

                    return myThis.clickLigne(val, lw);
                });
                //				(ligne).click(function (e: Event) { return myThis.clickLigne(val, new xxTableauLigneWrapper<T>(item)); });
            }


            if (myThis.dblClickLigne != undefined) {
                ligne.addEventListener('dblclick', (ev: MouseEvent) => { return myThis.dblClickLigne(val, new xxTableauLigneWrapper<T>(item)); });
                //(ligne).dblclick(function (e: Event) { return myThis.dblClickLigne(val, new xxTableauLigneWrapper<T>(item)); });
            }
     ...
     * @param tabVal
     */
    public ajouterDatas(tabVal: T[]): xxTableauWrapper<T> {
        let myThis: xxTableauWrapper<T> = this;
    ...
    public masquerPagination(): xxTableauWrapper<T> {
        let myThis: xxTableauWrapper<T> = this;

        cacherxElements(myThis.footerTableau, true);
        return myThis;
    }

    public afficherPagination(): xxTableauWrapper<T> {
        let myThis: xxTableauWrapper<T> = this;

        if (myThis.pagination > 0) {
            afficherxElements(myThis.footerTableau);
        }
        return myThis;
    }

    private delDatas(val: T[]): xxTableauWrapper<T> {
        let myThis: xxTableauWrapper<T> = this;
        val.forEach(function (a: T) {
            let supp: xxTableauItem<T>[] = myThis.mesItemsTous.filter(function (i: xxTableauItem<T>) { return a == i.donnee; });
    ...
    public supprimerDatas(val: T[]): xxTableauWrapper<T> {
        let myThis: xxTableauWrapper<T> = this;
        if( val!=null)
    ...
    */
    public supprimerDatasAll(): xxTableauWrapper<T> {
        let myThis: xxTableauWrapper<T> = this;
        let exportListe: T[] = [];
        myThis.dataContext.forEach(function (item: T) { exportListe.push(item); });
        myThis.dataContext.del(exportListe);
        /*  myThis.mesItemsTous.forEach(function (item: xxTableauItem<T>)
          {
              item.detruire();
          });*/
    ...
    public getColonnesBySelecteur(filtre: ((colonne: xxColonne<T>) => boolean)): xxColonne<T>[] {
        let myThis: xxTableauWrapper<T> = this;
        return myThis.getColonnes().filter(filtre);
    }

    public getColonneByClef(clef:number): xxColonne<T>{
        let myThis: xxTableauWrapper<T> = this;
     ...
     * @param colonne
     */
    public ajouterColonne(colonne: xxColonne<T>): xxTableauWrapper<T> {   //création de la zone titre de la colonne
        let myThis: xxTableauWrapper<T> = this;
     ...
     * @param colonne
     */
    public supprimerColonne(colonne: xxColonne<T>): xxTableauWrapper<T> {
        let myThis: xxTableauWrapper<T> = this;
        ...
        myThis.mesItemsTous.forEach(function (i: xxTableauItem<T>) {
...
//                i.rendered = false;
        ...
        myThis.mesColonnes = myThis.mesColonnes.filter(function (c: xxColonne<T>) { return c != colonne; });
        myThis.colonneTri = myThis.colonneTri.filter(function (c: xxColonne<T>) { return c != colonne; });
        return myThis;
    }
    public decalerColonnePositionFixe(colonne: xxColonne<T>, position: number ): xxTableauWrapper<T> {
    ...
    public ajouterColonnePositionFixe(colonne: xxColonne<T>, position: number = -1): xxTableauWrapper<T> {
        let myThis: xxTableauWrapper<T> = this;
        ...
        if (position == -1) {
            ...
            let mesColonneApres: xxColonne<T>[];
    ...
    public ajouterColonnePourItem(c: xxColonne<T>, item: xxTableauItem<T>, tab: xxTableauWrapper<T>) {
        ...
        if (item.rendered) {
            ...
            c.renderMethod(xElementHolder.fromHtmlElement(item.renderPlaces[c.cle]),
                item.donnee,
                new xxTableauLigneWrapper<T>(item),
                tab
            );
     ...
     * @param index
     */
    public setPagination(index: number): xxTableauWrapper<T> {
        let myThis: xxTableauWrapper<T> = this;
    ...
    public async filtrerParTexteExterne(s: string): Promise<xxTableauWrapper<T>> {
        let myThis: xxTableauWrapper<T> = this;
        return await myThis.filtrerParFonctionAsync((t) => { return myThis.filtreTexte(s, t); });
    }

    public async filtrerParFonctionAsync(fi: (t: T) => Promise<boolean>): Promise<xxTableauWrapper<T>> {
        let myThis: xxTableauWrapper<T> = this;
        let tabFiltre: xxTableauItem<T>[];
        const asyncFilter = async (arr: any[], predicate: (t: xxTableauItem<T>) => Promise<boolean>) => Promise.all(arr.map(predicate))
            .then((results) => arr.filter((_v, index) => results[index]));

        tabFiltre = await asyncFilter(myThis.mesItemsTous, (item: xxTableauItem<T>) => {
    ...
    public async filtrerParFonction(fi: (t: T) => boolean): Promise<xxTableauWrapper<T>> {
        let myThis: xxTableauWrapper<T> = this;
    ...
    private rafraichirCompteurs(): xxTableauWrapper<T> {
        let myThis: xxTableauWrapper<T> = this;
        if (myThis.compteurComplet != null) {
            //            myThis.placeCompteurPagination.changerTextVariable(myThis.getPageCourante());
            myThis.compteurComplet.changerTextVariable(new xLString('Page {0}/{1}').format([myThis.getPageCourante(), myThis.getNbPages()]));
    ...
    private SetMesItemsFiltres(tabFiltre: xxTableauItem<T>[]): xxTableauWrapper<T> {
    ...
    private filtrerParTexte(): xxTableauWrapper<T> {
        let myThis: xxTableauWrapper<T> = this;
    ...
    private ajouterColonneNoData(colonne: xxColonne<T>): xxTableauWrapper<T> {   //création de la zone titre de la colonne
        let myThis: xxTableauWrapper<T> = this;
        ...
        colonne.wrapInEntete = new xxWrapPanel({
        ...
        if (colonne.tooltipTitleLocalise != undefined) {
            xElementHolder.fromHtmlElement(colonne.entete).xxToolTip({
                initContent: colonne.wrapInEntete,
                toolTipContent: new xxLabel({ textLocalise: colonne.tooltipTitleLocalise }),
        ...
        //si je suis une colonne triable
        {
            ...
            if (colonne.renderTitre != undefined) {
                if (colonne.IsTriable)
                {
                    ...
                    btnTri = new xxBouton(optionsBoutonEntete);
        ...
        if (colonne.canDeleteColumn) {
            ...
            colonne.wrapInEntete.append(new xxBouton({
    ...
    private getColonneTrieVerrouille(): xxColonne<T> {
        let myThis: xxTableauWrapper<T> = this;
        let recherche_col_lock: xxColonne<T>[] = myThis.mesColonnes.filter(function (col: xxColonne<T>) { return col.IsVerrouTriPrincipal; });
        if (recherche_col_lock.length > 0)
            return recherche_col_lock[0];
        return null;
    }

    private setTriPrincipal(colonne: xxColonne<T>, withToggle: boolean): xxTableauWrapper<T> {
        let myThis: xxTableauWrapper<T> = this;
        ...
        let colonne_lock: xxColonne<T> = myThis.getColonneTrieVerrouille();
        let colonne_source: xxColonne<T> = null;
        ...
        if (colonne_lock != null && colonne != colonne_lock) //on a demandé u trie sur une autre colonne de la colonne verrouillé
        {
            ...
            myThis.colonneTri
                .filter(function (col: xxColonne<T>) { return col != colonne && col.IsTriable; })
                .forEach(function (col: xxColonne<T>) {
    ...
    private addTriSupplementaire(colonne: xxColonne<T>, withToggle: boolean): xxTableauWrapper<T> {
        let myThis: xxTableauWrapper<T> = this;
        let tempTypeTri: enumTypeTri = colonne.getTypeTri();
    ...
    private trierlesDonnees(): xxTableauWrapper<T> {
        let myThis: xxTableauWrapper<T> = this;

        if (myThis.colonneTri.length > 0) {

            myThis.mesItemsFiltres.sort(
                function (a: xxTableauItem<T>, b: xxTableauItem<T>): number {
                    let valeur: number = 0;
                    let ixColonneEnCours = 0;
                    while (valeur == 0 && ixColonneEnCours < myThis.colonneTri.length) {
                        let col: xxColonne<T> = myThis.colonneTri[ixColonneEnCours];
    ...
    private rafraichirContenuTableau(): xxTableauWrapper<T> {
    ...
    private attacherALalisteGraphique(debut: number, pagination: number): xxTableauWrapper<T> {
        let myThis: xxTableauWrapper<T> = this;
        let itemPrecedent: xxTableauItem<T> = null;
        let withGroup: boolean = false;
        ...
        let elementsAAfficher: xxTableauItem<T>[];
        ...
        //    elementsAAfficher.forEach(function (item: xxTableauItem<T>, index: number) {
        ...
        //            td.classList.add("xxTableauGroup");
        ...
        //else {
        //    //ajout  des éléments
        //    elementsAAfficher.forEach((item: xxTableauItem<T>, index: number) => {
        //        //puis j'ajoutes l'élément
        //        myThis.monTableau.tBodies[0].appendChild(item.ligne);
        ...
        elementsAAfficher.forEach((item: xxTableauItem<T>) => { if (!item.rendered) item.render(); });

        return myThis;
    }

    private viderLaListeGraphique(): xxTableauWrapper<T> {
        let myThis: xxTableauWrapper<T> = this;
     ...
     */
    private createLigneDetail(item: xxTableauItem<T>, index: number): void {
        let myThis: xxTableauWrapper<T> = this;

        let detailLigne: HTMLTableRowElement;
        detailLigne = myThis.monTableau.insertRow();
        detailLigne.classList.add('xxTableauLigneDetail');
        item.ligneDetail = detailLigne;
        item.Detailrendered = false;

        let ligneTab: xxTableauLigneWrapper<T> = myThis.getLigneByValue(item.donnee);
        ...
        item.renderDetail = () => {
            ...
            tdDetail.classList.add("xxTableauDetail");
            tdDetail.colSpan = myThis.mesColonnes.length;
            tdDetail.id = "xxTableauDetail_" + index;
    ...
    //    let myThis: xxTableauWrapper<T> = this;
    //    myThis.mesColonnes[0].renderMethod = function (place: xElementHolder, valeur: T, lw: xxTableauLigneWrapper<T>) {
    ...
    //private AjoutEnfantListe(niveau: number, enfants: T[], listDonne: T[], list: xxTableauItem<T>[], getEnfants: (a: T) => T[], getParent: (a: T) => T) {
    //    let myThis: xxTableauWrapper<T> = this;
    //    if (enfants != null && enfants.length > 0)
    //        enfants.forEach(e => {
    ...
    //                let renderPlaces: Dictionnaire<HTMLTableDataCellElement> = {};
    //                item = new xxTableauItem<T>(ligne, renderPlaces, e, myThis);
    //                let css = "";
    //                css += niveau.toString();
    //                //if (niveau % 2 == 0)
    //                //	css = "pair"
    ...
    //        })
    //}
    public addClass(c: string): xxTableauWrapper<T> {
        let myThis: xxTableauWrapper<T> = this;
    ...
    public removeClass(c: string): xxTableauWrapper<T> {
        let myThis: xxTableauWrapper<T> = this;
...
class xxTableauItem<T> {
    ...
    constructor(ligneComplete: HTMLTableRowElement, rPlaces: Dictionnaire<HTMLTableDataCellElement>,
        inDonnee: T, leTableau: xxTableauWrapper<T>) {
        let myThis: xxTableauItem<T> = this;
        ...
        myThis.render = () => {
            ...
            leTableau.mesColonnes.forEach(function (c: xxColonne<T>) {  //nettoyage de toutes les cellules
