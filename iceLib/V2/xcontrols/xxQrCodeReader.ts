...
class xxQrCodeReader implements iXElement {
    ...
    constructor(o: optionsQrCodeReaderBinding|optionsQrCodeChange) {
        let myThis: xxQrCodeReader = this;
                ...
                o.binding.visibility.bind((s) => {
                    switch (s) {
                        case enumVisibility.afficher:
                            afficherxElements(myThis.myCanvas);
                            break;
                        case enumVisibility.masquer:
                            cacherxElements(myThis.myCanvas, false);
                            break;
                        case enumVisibility.masquerAvecCollapse:
                            cacherxElements(myThis.myCanvas, true);
                            break;
                    }
                })
            }
        ...
        if (o.onError != undefined) {
            ...
            myThis.onError = (err) => {
                console.log(err);
                xOutils.afficherMessageAlertifyLocaliseError('Appareil non disponible');
            };
        }

        myThis.myCanvas = new xCanvas({class:"xxQrCodeReader "+o.class,id:o.id});
    ...
    public start() {
        ...
        if (myThis.video != null) {
            xOutils.afficherMessageAlertifyLocaliseLog('démarrage');
            ...
            xOutils.afficherMessageAlertifyLocaliseError('Vidéo non disponible');
        }
     ...
     private tick() {
         ...
         if (myThis.video.readyState === myThis.video.HAVE_ENOUGH_DATA) {
            ...
            //   xOutils.afficherMessageAlertifyError('erreur');
        }
         }
