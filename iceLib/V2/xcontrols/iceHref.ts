...
import { ice2Boxer } from './ice2Boxer';
...
export class iceHref extends iceElement
{
    constructor(options?: OptionsHref)
    {
        ...
        this.addClass("iceHref");

        let classCouleur: string = "couleurzeus_bleu";

        if (options.optionsAffichage != undefined)
        {
            iceStyle.AppliquerOptionsAffichage(this, options.optionsAffichage);
        ...
        if (options.typeOuverture == enumTypeOuvertureHref.Boxer)
        {
            ...
            this.y.onclick= ()=>
            {
                let boxer: any = new ice2Boxer(
                    {
                        initContent: new iceIFrame({ src: options.url })
                    });
                boxer.afficher()
            }
        }
