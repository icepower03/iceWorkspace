...
import { ice2Label, OptionsLabel } from './ice2Label';
...
import { Icone, IconeSvg, enumIconeSvg } from '../iceIcones';
...
export interface optionBouton {
    ...
    textVariable?: string;
    disabled?: boolean;
    icone?: Icone | iceIconeAvecAction;
    click: (showButton?: () => void, ceBouton?: ice2Bouton) => void;
    dblclick?: (showButton?: () => void, ceBouton?: ice2Bouton) => void;

    shiftClick?: (showButton?: () => void, ceBouton?: ice2Bouton) => void;
    touchLong?: (showButton?: () => void, ceBouton?: ice2Bouton) => void;
    confirm?: {
...
export class ice2Bouton implements iXElement {
    // Graphiques 
    private mainDiv: ice2ContainerEvent;
    private secondDiv: iceDiv;    
    private label: ice2Label;
    ...
    private icone: Icone | iceIconeAvecAction
    private click: (showButton: () => void,ceBouton:ice2Bouton) => void;
    private dblclick: (showButton: () => void, ceBouton: ice2Bouton) => void;
    private fullHeight: boolean;
    private fullWidth: boolean;

    private shiftClick: (showButton: () => void, ceBouton: ice2Bouton) => void;
    private touchLong: (showButton: () => void, ceBouton: ice2Bouton) => void;
    ...
    constructor(option: optionButton) {
        ...
        myThis.class = "ice2BoutonContainer";
        myThis.text = "";
        myThis.title = "";
        ...
        if (option.titleLocalise != undefined)
            myThis.title = new iceLString(option.titleLocalise).text;
        else
            myThis.title = option.titleVariable;

        if (!myThis.isOptionLabelled(option)) {
            if (option.textLocalise != undefined)
                myThis.text = new iceLString(option.textLocalise).text; 
            ...
            if (option.binding.visibility != undefined)
            {
                switch (option.binding.visibility.Value)
                {
                    case enumVisibility.afficher:
                        affichericeElements(myThis.mainDiv);
                        break;
                    case enumVisibility.masquer:
                        cachericeElements(myThis.mainDiv, false);
                        break;
                    case enumVisibility.masquerAvecCollapse:
                        cachericeElements(myThis.mainDiv, true);
                        break;
                }
    ...
    private createButton() {
        ...
        myThis.mainDiv = new ice2ContainerEvent({
        ...
        myThis.secondDiv = new iceDiv({
            class: "ice2BoutonContent" + myThis.getSecondClass()
        });
        ...
        else if (myThis.type == enumTypeBouton.TexteHorsBouton)
        {
            if (myThis.optionLabel != undefined)
                myThis.label = new ice2Label(myThis.optionLabel);
            else
                myThis.label = new ice2Label({ textVariable: myThis.text });
        ...
        {
            iceStyle.AppliquerOptionsAffichage(myThis.secondDiv, myThis.optionsAffichage);
        }
    ...
    private reGenerateContentSecondeDiv()
    {
        ...
        if (myThis.type == enumTypeBouton.Standard)
        {
            myThis.span = new iceSpan({ textVariable: myThis.text });
    ...
    private clickBehaviour(showButton: () => void, shiftClick: boolean): void 
    {
        ...
        switch (myThis.confirmBehaviour) 
        {
            ...
            case enumComportementBouton.ActionDifferee:
                ...
                p = new Promise((resolve, reject) => {
                    ...
                    let btnAnnuler = new ice2Bouton(opD);
            ...
            case enumComportementBouton.ActionAConfirmer:
                ...
                p = new Promise((resolve, reject) => {
                    ...
                    let btnconfirm = new ice2Bouton(opD);
            ...
            case enumComportementBouton.ValidationBloquante:
                ...
                p = new Promise((resolve, reject) => {
                    ...
                    iceOutils.afficherMessageConfirmationLocalise(monMsg, false, (selection: boolean) => { if (selection) { resolve({}); } else { reject() } });
                });
                break;
        }
        p.then(() =>
        ...
        }, () => 
        {
            ...
            iceOutils.afficherMessageAlertifyError("annulé");
    ...
    public hideButton(collapse?: boolean): void {
        let myThis: ice2Bouton = this;
        cachericeElements(myThis.mainDiv, collapse != undefined ? collapse : false);
    }
    ...
    public getIcone(): Icone | iceIconeAvecAction
    {
        let mythis: ice2Bouton = this;
        return mythis.icone;
    }
    ...
    * */
    ...
    public removeAttribute(strAttr: string): ice2Bouton {
        let myThis: ice2Bouton = this;
    ...
    public setAttribute(strAttr: string, valeur: string): ice2Bouton {
        let myThis: ice2Bouton = this;
    ...
    public setClick(clickCb: (showButton?: () => void, ceBouton?: ice2Bouton) => void): ice2Bouton{
        let myThis: ice2Bouton = this;
       
        myThis.click = clickCb;
        return myThis;
    }
    public setShiftClick(sclickCb: (showButton?: () => void, ceBouton?: ice2Bouton) => void): ice2Bouton {
        let myThis: ice2Bouton = this;
