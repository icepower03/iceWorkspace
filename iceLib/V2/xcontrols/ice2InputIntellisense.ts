...
export class ice2InputIntellisense implements iXElement {
    private divContent: iceDiv;
    private divContentEditable: iceDiv;
    private tooltipAutoComplete: ice2ToolTip;
    private listeMotsAutoComplete: string[];
    private listAutoComplete: ice2ListWrapper<string>;
    private listEventContainers: ice2ContainerEvent[];
    ...
    public width(parame?: string | number): void | number {
        let myThis: ice2InputIntellisense = this;
    ...
    constructor(options: OptionsInputIntellisense) {
        ...
        if (options.placeHolderlocalise != undefined) {
            placeHolder = new iceLString(options.placeHolderlocalise).text;
        }
        else {
        ...
        myThis.divContent = new iceDiv({ class: 'ice2InputIntellisense' });
        myThis.divContentEditable = new iceDiv({ class: 'zoneEditable' });
        myThis.divContentEditable.y.setAttribute('contenteditable', 'true');
        myThis.divContent.asHolder.append(new iceDiv({ class: 'zoneInfo', title: new iceLString("Assistant : commencez la saisie et obtenez des propositions pertinentes selon votre utilisation.").text }));
                    ...
                    if (lastSpaceIndex > -1) {
                        ...
                        if (myThis.debutMot.length >= 3) {
                            let listeMotsFiltres: string[] = myThis.listeMotsAutoComplete.filter(function (w) {
                                return iceOutils.rechercheString(myThis.debutMot, [w]);
                            });
                            ...
                            if (listeMotsFiltres.length > 0) {
                                ...
                                if (myThis.listAutoComplete == null) {
                                    myThis.listAutoComplete = new ice2ListWrapper<string>({
                                        donnees: [],
                                        class: "list_auto_complete",
                                        renderItem: function (p, item) {
                                            let label: ice2Label = new ice2Label({ textVariable: item });
                                            label.setSurbrillance(myThis.debutMot);

                                            let eventContainer: ice2ContainerEvent = new ice2ContainerEvent({
                                                class: "suggestionMot",
                                                initContent: label,
                                                onClick: function (e) {
                                ...
                                if (myThis.tooltipAutoComplete == null) {
                                    myThis.tooltipAutoComplete = new ice2ToolTip({
                                        class: "ice2InputIntellisense_Tooltip",
                                        toolTipContent: myThis.listAutoComplete,
                                        initContent: new iceDiv({ class: "divSuggestionMots" })
                                    });
