...
export class xInputText extends iceElement {
    ...
    constructor(options: OptionsInput) //|OptionsInputBind
    {
                ...
                options.binding.visibility.bind((s: enumVisibility) => {
                    switch (s) {
                        ...
                        case enumVisibility.masquer:
                            cachericeElements(myThis, false);
                            break;
                        case enumVisibility.masquerAvecCollapse:
                            cachericeElements(myThis, true);
                            break;
                    }
                })
            }
        ...
        let classStyles: string = "iceInput";
        ...
        {
            iceStyle.AppliquerOptionsAffichage(myThis, options.optionsAffichage);
        }
        ...
        if (myThis.isNumeric) {
            if (valeur.length > 0) {
                    ...
                    if (isNaN(parseFloat(valeurEnCour))) {
                        if (valeurEnCour == ".") {
                            if (myThis.decimalSeparator && !seprationDecimal && i != 0 && (valeur[i - 1] != "-" && valeur[i - 1] != "+"))
                                ...
                                iceOutils.afficherMessageAlertifyLocaliseError("Le caractère . n'est pas autorisé ici");
                                return false;
                            }
                        }
                            ...
                            iceOutils.afficherMessageAlertifyLocaliseError("Votre saisie est incorrecte");
                            return false;
                        }
                    }
                }
                ...
                if (myThis.nbDecimal != undefined) {
                    if (valeur.substr(valeur.indexOf('.'), valeur.length).length - 1 > myThis.nbDecimal) {
                        iceOutils.afficherMessageAlertifyError(new iceLString('Le nombre de décimales {0} est dépassé.').format([myThis.nbDecimal]));
                        return false
                    }
                }

                if ((myThis.max != undefined) && (valeurNumber > myThis.max)) {
                    iceOutils.afficherMessageAlertifyError(new iceLString('La valeur maximale de {0} est dépassée.').format([myThis.max]));
                    return false;
                }
                ...
                if (myThis.nbDigits != undefined && myThis.nbDigits < uniquementlesChiffre.length) {
                    iceOutils.afficherMessageAlertifyError(new iceLString('Le nombre de chiffres maximal de {0} est dépassé.').format([myThis.nbDigits]));
                    return false;
                }
            ...
            if (myThis.longueurMax != undefined) {
                if (valeur.length > myThis.longueurMax) {
                    iceOutils.afficherMessageAlertifyLocalise("longueur maxi:" + myThis.longueurMax, ETypeAlertify.log);
                    return false;
                }
            }
        }
        ...
        if (myThis.isNumeric) {
            if (valeur.length > 0) {
                ...
                if ((myThis.min != undefined) && (valeurNumber < myThis.min)) {
                    iceOutils.afficherMessageAlertifyError(new iceLString("La valeur minimale de {0} n'est pas atteinte").format([myThis.min]));
                    return false;
                }
            }
        }
