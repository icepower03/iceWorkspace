...
export class xxListWrapper<T> implements iXElement {
    ...
    public listeItems: xxListItem<T>[];
    private monDomContainer: xxWrapPanel | xxStackPanel | xxGrid;
    ...
    private regroupementUniqueBy_Dico: { [key: string]: { headerZone?: iXElementHolder, listeItem: xxListItem<T>[] } };

     // Affichage en grid //
    private affichageEnGrid: boolean = false;
    private listGridItem:  xxGridItem []=[];
    ...
    constructor(inOptions: OptionsList<T>) {
        let myThis: xxListWrapper<T> = this;
        ...
        if (myThis.affichageEnGrid==true)
        {
            ...
            myThis.monDomContainer = new xxGrid({
                id:  inOptions.id,
                colonnes_auto: "1fr",
                gridGap: myThis.gridGap, fullWidth: true,
                padding: false,
                class:"xxListPrincipal " + addClass
            });
        }else{ 
            if (myThis.modeHorizontal==true ) {
                myThis.monDomContainer = new xxWrapPanel({ class: "xxListPrincipal " + addClass,
                id: inOptions.id,
            ...
            {
                myThis.monDomContainer = new xxStackPanel({ class: "xxListPrincipal " + addClass, id: inOptions.id, gap: myThis.gap });
            } 
        }
        myThis.listeItems = [];
        myThis.listGridItem = [];
            ...
            myThis.listeItems.forEach(a => {
                ...
                if (myThis.affichageEnGrid) {
                    if (myThis.listGridItem && myThis.listGridItem[iIndex] != null) {
                        let gridItem: xxGridItem = myThis.listGridItem[iIndex];
                        ...
                        if (visibilite) {
                            ...
                            afficherxElements(gridItem);
                        } else {

                            cacherxElements(gridItem);
                        }
                    }
                }
                iIndex++;
        ...
        if (myThis.regroupementUniqueBy_GroupBy != null) {
            Object.keys(myThis.regroupementUniqueBy_Dico).forEach((key) => {
                if (myThis.regroupementUniqueBy_Dico[key].listeItem.some(t => t.visible.Value)) {
                    afficherxElements(myThis.regroupementUniqueBy_Dico[key].headerZone);
                    ...
                    cacherxElements(myThis.regroupementUniqueBy_Dico[key].headerZone, true);
            });
        }
    ...
    public refreshTri() {
        ...
        if (myThis.monTri != enumTypeTri.aucun) {
            ...
            myThis.listeItems.sort(
                function (a: xxListItem<T>, b: xxListItem<T>): number {
                    if (myThis.monTri == enumTypeTri.desc) {
                        //swap si tri descendant
                        let temp: xxListItem<T>;
                        temp = b;
                        b = a;
                        a = temp;
                    }
        ...
        myThis.listeItems.forEach(function (item: xxListItem<T>, index) {
    ...
    public setFiltre(inFiltre: (obj: T) => boolean): xxListWrapper<T> {
        let myThis: xxListWrapper<T> = this;
        myThis.filtreActif = inFiltre;
        return myThis;
    }

    public changerTri(tri?: enumTypeTri): xxListWrapper<T> {
        let myThis: xxListWrapper<T> = this;
    ...
    // suppression //
    public supprimerItem(item: T): xxListWrapper<T> {
        return this.supprimerItems([item]);
    }

    public supprimerItems(item: T[]): xxListWrapper<T> {
        let mythis: xxListWrapper<T> = this;
    ...
    public supprimerItemsAll(): xxListWrapper<T> {
        let myThis: xxListWrapper<T> = this;
        myThis.supprimerItems(myThis.getAllData());
        return myThis;
    }
    private delData(items: T[]): xxListWrapper<T> {

        let mythis: xxListWrapper<T> = this; 
        items.forEach(function (v: T) {

            mythis.listeItems.forEach(function (val: xxListItem<T>, index: number) {
                if (v != undefined) {
                    if (mythis.equals(val.donnee, v)) {
                        ...
                        if (mythis.affichageEnGrid) {
                            if (mythis.listGridItem[index] != null) {
                                let gridItem: xxGridItem = mythis.listGridItem[index];
                                let grid: xxGrid = mythis.monDomContainer as xxGrid;
    ...
    // Ajoute //
    public ajouterItems(items: T[]): xxListWrapper<T> {
        let mythis: xxListWrapper<T> = this;
    ...
    private   appendToContener(element:iXElement):void{
        ...
        if (this.affichageEnGrid == true) {
            let DomContainer: xxGrid = myThis.monDomContainer as xxGrid;
            DomContainer.append([element]); 
        } else {
            if (this.modeHorizontal==true) { 
                let DomContainer: xxWrapPanel = myThis.monDomContainer as xxWrapPanel;
                DomContainer.append(element);
            } else {
                let DomContainer: xxStackPanel = myThis.monDomContainer as xxStackPanel;
                    DomContainer.append(element);
            }
        }
    }
    private addData(items: T[]): xxListWrapper<T> {
        let myThis: xxListWrapper<T> = this;
        let nbcols: number = 1;
        ...
        if ((items.length <= myThis.LIMITE_NUMBER || !myThis.limite)) {
            items.forEach(function (v: T, index: number) {
                ...
                if (myThis.modeUnique) {
                    let listeFiltree: xxListItem<T>[] = myThis.listeItems.filter(function (val: xxListItem<T>) {
                ...
                if (ajout) {
                    ...
                    let tdiv: xDiv = null;
                    let tGridItem: xxGridItem = null;
                    
                    if (myThis.affichageEnGrid == true) { 
                        tdiv = new xDiv({ class: "xxListItem xxListItemGrid", id: idTempo != "" ? "xxListItem_" + idTempo : null });
                        if (index == items.length - 1) { nbcols = myThis.nbColonnesGrid - (index % myThis.nbColonnesGrid); }

                        else { nbcols = 1; }
                        nbcols = 1; 
                        tGridItem = new xxGridItem({
                        ...
                        tdiv = new xDiv({ class: "xxListItem", id: idTempo != "" ? "xxListItem_" + idTempo : null });
                    ...
                    let item: xxListItem<T> = new xxListItem<T>(tdiv, v, visible, myThis.renderItem, idTempo);
                        ...
                        if (NameGrouptempo != null) {
                            ...
                            if (!myThis.regroupementUniqueBy_Dico[NameGroup]) {
                                myThis.regroupementUniqueBy_Dico[NameGroup] = {
                                    headerZone: new xDiv({ class: "xxListGroupeHeader", id: "xxListGroupeHeader_" + NameGroup }).asHolder, listeItem: [] };
            ...
            myThis.appendToContener(new xxLabel({ textLocalise: "Trop de données, veuillez affiner votre recherche.", type: enumTypeLabel.information }))
    ...
    private GenerateGroupGlobal() {
        let myThis: xxListWrapper<T> = this;
        let itemPrecedent: xxListItem<T> = null;
    ...
    private GenerateLibelleSiVide() {
        ...
        if (mythis.LibelleSiVide != null && mythis.LibelleSiVide != "") {
            mythis.appendToContener(new xxLabel({
            ...
            // Permet de Force l'invisibilité du xxLabel a l'init
            mythis.affichageLabelSiVide.Value = enumVisibility.afficher;
            mythis.affichageLabelSiVide.Value = enumVisibility.masquerAvecCollapse;
            if (mythis.listeItems.length <= 0)
                mythis.affichageLabelSiVide.Value = enumVisibility.afficher;
    ...
    public getAllData(): T[] {
        let myThis: xxListWrapper<T> = this;
        let retour: T[] = [];
        myThis.listeItems.forEach(function (item: xxListItem<T>) {
    ...
    public render(): void {
        let myThis: xxListWrapper<T> = this;
        this.listeItems.forEach(function (item: xxListItem<T>, index: number) {
            item.render();
        });
        this.filtrer();
    }
    ...
    public addClass(strClasses: string): xxListWrapper<T> {
        let myThis: xxListWrapper<T> = this;
    ...
    public removeClass(strClasses: string): xxListWrapper<T> {
        let myThis: xxListWrapper<T> = this;
...
class xxListItem<T>{
    ...
    constructor(rPlace: xDiv, inDonnee: any, visible: boolean, renderMethod: (place: xElementHolder, valeur: T, id: string) => void, id: string) {
        let mythis: xxListItem<T> = this; 
        mythis.visible = new BindableObject(visible);
        mythis.visible.bind((visible) => {
            //déclenché que si la valeur a changé
            if (!visible) {
                cacherxElements(mythis.renderPlace, true);
            }
            else {
                afficherxElements(mythis.renderPlace)
            }
        });
