...
export class xxGridItem implements iXElement {
    ...
    private setProperties() {
        ...
        if (myThis.elementPrincipalItem != null) {
            ...
            myThis.elementPrincipalItem = new xDiv({ id: myThis.id });
        }

        myThis.elementPrincipalItem.addClass("xxGridItem");
        ...
        if (myThis.optionsAffichage != undefined)
        {
            ...
            xStyle.AppliquerOptionsAffichage(myThis.elementPrincipalItem, myThis.optionsAffichage);
...
export class xxGrid {
   public  Length():number {
    ...
    private mesElements: ObservableCollection<xxGridItem> = new ObservableCollection<xxGridItem>();
    ...
    private setProperties() {
        ...
        if (xMaths.isNumeric(myThis.autoColonnes)) {
        ...
        if (xMaths.isNumeric(myThis.autoLignes)) {
        ...
        if (myThis.optionsAffichage != undefined)
        {
            ...
            xStyle.AppliquerOptionsAffichage(myThis.elemPrincipal, myThis.optionsAffichage);
   ...
   /*     if (a.msGridRow != undefined && navigator.userAgent.indexOf('Edge/') <= 0) {
    ...
    public append(items: (xxGridItem | iXElement)[]): xxGrid {
        let myThis: xxGrid = this;

        let lstGridItems: xxGridItem[] = [];

        items.forEach(a => {
            if (myThis.isxxGridItem(a)) {

                lstGridItems.push(a);

            }
            else {
                lstGridItems.push(new xxGridItem({cssOnly:true, content:a}));
            }
        }
        );
    ...
    public supprimer(items: xxGridItem[]) {
        let myThis: xxGrid = this;
    ...
    constructor(o: OptionsGrid) {
        ...
        myThis.elemPrincipal = new xDiv({ id: o.id,drag:o?.drag, class: "xxGridMaster " + strClassAuto + " " + myThis.class });
    ...
    private isxxGridItem(obj: iXElement | xxGridItem): obj is xxGridItem {
        return (<xxGridItem>obj).cssOnly !== undefined;
    }

    public appendMatrix(tabelem: (iXElement | iXElementGridPoperty)[][]): xxGrid {
        let myThis: xxGrid = this;
        return myThis.appendMatrixStd(tabelem, false);

    }
    /*
    public appendMatrixCssOnly(tabelem: (iXElement | iXElementGridPoperty)[][]) :xxGrid{
        let myThis: xxGrid = this;
    ...
    */
    ...
    private appendMatrixStd(tabelem: (iXElement | iXElementGridPoperty)[][],cssOnly:boolean):xxGrid {
        let myThis: xxGrid = this;
        let tab: xxGridItem[] = [];
                ...
                ligne.forEach((elem: (iXElement | iXElementGridPoperty), indexColonne) => {
                    if (elem != null) {
                        if (myThis.isGridProperty(elem)) {
                            ...
                            tab.push(new xxGridItem({ content: elem.xelem, rowStart: indexLigne + 1, colStart: indexColonne + decalageColonne + 1, nbCols: elem.nbCols, nbRows: elem.nbRows, class:elem.class }));
                            ...
                            if (cssOnly) {
                                tab.push(new xxGridItem({ content: elem,cssOnly:true }));
                            }
                            else {
                                tab.push(new xxGridItem({ content: elem, rowStart: indexLigne + 1, colStart: decalageColonne + indexColonne + 1 }));
                            }
                        }
    ...
    public cacher(collapse?: boolean): xxGrid {
        let myThis: xxGrid = this;
    ...
    public setAutoColonnes(autoColonnes: string): xxGrid {
        let myThis: xxGrid = this;
    ...
    public removeClassFromAllElements(laclasse: string): xxGrid {
        let myThis: xxGrid = this;
    ...
    public addClassToElement(colstart: number, laclasse: string): xxGrid {
        let myThis: xxGrid = this;
    ...
    public removeClassFromElement(colstart: number, laclasse: string): xxGrid {
        let myThis: xxGrid = this;
