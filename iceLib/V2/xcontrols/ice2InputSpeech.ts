...
export class ice2InputSpeech implements iXElement
{
    private elementPrincipal: iceDiv;

    private iceInput: xInputText;

    constructor(options: OptionsInput)

    {
        this.elementPrincipal = new iceDiv({
            class: "ice2InputSpeech " + (options.class ?? ''),
            id: options.id
        });
      ...
      this.iceInput  = new xInputText(options);
      this.iceInput.addClass("texteInputSpeech");

        let myThis: ice2InputSpeech = this;
        // let xInputPourcent: ice2Label = new ice2Label({textVariable:"%"});
        this.elementPrincipal.asHolder.append(this.iceInput);
        this.elementPrincipal.asHolder.append(new ice2Bouton({
            ...
            click: function (cb)
            {
                ...
                recognition.onresult = function (event:any)
                {
                    ...
                    {
                        myThis.iceInput.setValueFireEvent(event.results[i][0].transcript);
    ...
    public setValue(texte: string): void {// marche pas 
        let myThis: ice2InputSpeech = this;
        myThis.iceInput.setValue(texte);
    }
