...
import { iXElement } from './V2/iceBase';
import { addClass, removeClass } from './V2/iceDomUtils';

export function cachericeElements(j: iXElement, collapse?: boolean) {
...
export function affichericeElements(j: iXElement) {
    if (j?.y != null) {
        removeClass("xdisabled xInvisible", j);
        
    }
}
export function videriceElements(j: iXElement) {
    if (j?.y != null) {
