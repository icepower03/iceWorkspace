// @ts-nocheck
import { xClass } from './V2/iceBase';
declare const iXElement: any;
declare const iXElementHolder: any;
declare const iceDiv: any;
declare const iceLString: any;
declare const ice2Dialog: any;
declare const ice2BloqueEcran: any;
declare const ice2PageWrapper: any;
...
export class iceOutils {
    ...
    public static replaceAll(str: string, find: string, replace: string): string {
        return str.replace(new RegExp(iceOutils.escapeRegExp(find), 'g'), replace);
    }
    ...
    */
    ...
    static JqueryAjaxCall(pageUrl: string, parameter: any, functionRetour: (success: any, context?: any) => void, context?: any, functionError?: (data: any, success: any, error: any) => void): void {
        $.ajax({
            ...
            error: functionError != null ? functionError : function (data, success, error) {
                    ...
                    iceOutils.afficherMessageAlertify("at " + pageUrl + ", Error :" + data.responseText, ETypeAlertify.error);
    ...
    static JqueryAjaxCallPromiseForTsProxy<T>(pageUrl: string, parametreAppel: any, maPage?: ice2PageWrapper): Promise<T> {
    ...
    static JqueryAjaxCallPromiseForTs<T>(pageUrl: string, parametreAppel: any, maPage?: ice2PageWrapper): Promise<T> {
    ...
    static JqueryAjaxCallPromiseForTsJsonWithErrors<T>(pageUrl: string, parametreAppel: any, 
        withJSON: boolean, maPage?: ice2PageWrapper, catchPerso?: (s: string) => void): Promise<T> {


        let promesseRetour = new Promise<T>((resolve, reject) => {
            let promesse = iceOutils.JqueryAjaxCallPromiseForTsJson<retourJsonWithErrors<T>>(pageUrl, parametreAppel, withJSON, maPage);
        ...
        if (catchPerso != undefined) {
            ...
            promesseRetour.catch(
                (err: string) => {
                        ...
                        iceOutils.afficherMessageAlertifyError(err);
    ...
    /**
            ...
            * @param settings
            */
    static JqueryAjaxCallPromiseForTsJsonDistant<T>(parametreAppel: any, maPage?: ice2PageWrapper): Promise<T> {
        let divws: iceDiv = undefined;
        ...
        let promesseZeus: Promise<T> = new Promise<T>((resolve, reject) => {
           ...
           let cryptageEtJeton: Promise<Response> = iceOutils.JqueryAjaxCallPromise2(iceOutils.convertDevUrlToRelativeUrl("~/xModules/ModuleZeus.aspx/Before_requestWebService_Echange"), parametreAppel, { async: true });
            ...
            cryptageEtJeton.then(async (retourBefore: Response)=>
            {
                ...
                let requestDistante: Promise<Response> = iceOutils.JqueryAjaxCallPromise2(retourBeforeObjet.URL_Zeus_Clinique + "/xModules/ModuleZeus.aspx/requestWebService_Echange", { data_echange: retourBeforeObjet }, { async: true });

                requestDistante.then(async (retourRequest: Response)=>
                {
                    let retourAfterObjet = (await retourRequest.json()).d;
                    let decryptWithKey: Promise<Response> = iceOutils.JqueryAjaxCallPromise2(iceOutils.convertDevUrlToRelativeUrl("~/xModules/ModuleZeus.aspx/After_requestWebService_Echange"), { data_echange: retourAfterObjet }, { async: true });
                    ...
                    decryptWithKey.catch(function (erreur: string) {
                        ...
                        iceOutils.afficherMessageAlertify("Erreur lors du déchiffrement de la réponse d'un appel vers l'URL ", ETypeAlertify.error);
                ...
                requestDistante.catch(function (erreur: string) {
                    ...
                    iceOutils.afficherMessageAlertify("Erreur lors d'un appel vers l'URL " + retourBeforeObjet.URL_Zeus_Clinique, ETypeAlertify.error);
            ...
            cryptageEtJeton.catch(function (erreur: string) {
                ...
                iceOutils.afficherMessageAlertify("Erreur lors de la création du jeton d'échange", ETypeAlertify.error);
        ...
        promesseZeus.catch(
            function (err: any) {
                ...
                iceOutils.afficherMessageAlertify(err.responseJSON != undefined ? err.responseJSON.Message : err, ETypeAlertify.error);
       ...
       static JqueryAjaxCallPromiseForTsJson<T>(pageUrl: string, parametreAppel: any, withJSON: boolean, maPage?: ice2PageWrapper): Promise<T> {
       return iceOutils.JqueryAjaxCallPromiseForTsJsonFetch(pageUrl, parametreAppel, withJSON, maPage);
   }
 
    static JqueryAjaxCallPromiseForTsJsonFetch<T>(pageUrl: string, parametreAppel: { methode: string }, withJSON: boolean, maPage?: ice2PageWrapper): Promise<T>
    {

        pageUrl = iceOutils.convertDevUrlToRelativeUrl(pageUrl);
        let divws: iceDiv = undefined;
        if (maPage != undefined && maPage != null)
        {
            divws = maPage.ajouterWS();
        ...
        {
            iceOutils.JqueryAjaxCallPromiseFetch(pageUrl, parametreAppel, { async: true }).then(async (retour: Response) =>
            {
                ...
                catch (erreur)
                {
                    if (monretourstring?.d != null && monretourstring.d.startsWith("InvalidSession:-")) //si c'est une erreur de sesison on ne la remonte a l'utilisateur (pour ne pas avoir de msg rouge avant la deconnection)
                        ...
                        iceOutils.AfficheErreurAjax("Erreur des données de retour de la méthode Ajax " + parametreAppel.methode, erreur, divws);
                }
            }).catch((erreur) =>
            {
                iceOutils.AfficheErreurAjax("Erreur pendant l'appel de la méthode Ajax " + parametreAppel.methode, erreur, divws);
                reject(erreur);
            });
        })
        ...
        {
            iceOutils.AfficheErreurAjax("Erreur catch 'impossible' promesse " + parametreAppel.methode, erreur, divws);
        });
        return promesse;
    }

    private static AfficheErreurAjax(erreur_info: string, erreur: any, divws: iceDiv) 
    {
        ...
        //{
        //}
        iceOutils.afficherMessageAlertify(erreur_info, ETypeAlertify.error);
    }

    private static ajouterWS(): iceDiv {

        let retour: iceDiv = new iceDiv({ class: 'Global_wsLineElement' });
        return retour;
    }
    ...
    static afficherMessageAlertify(message: string, type: ETypeAlertify, options: alertify.IProperties = {}): void {
        ice2Dialog.afficherMessageDialog(message, type, options);
    }
    ...
    {
        ice2Dialog.afficherMessageDialogContent(content, type, options);
    }
    ...
    static afficherMessageConfirmation(message: string, afficherOuiNon: boolean, delegueReponse: (sucess?: boolean) => void, type_messagebox?: etype_messagebox/*, bloquerBtnValider: boolean = false, NomDeBoutonCustom?: { ok: string, cancel: string }*/, sansBoutons?: boolean): void {
        ice2Dialog.afficherMessageConfirmation(message, afficherOuiNon, delegueReponse, type_messagebox, sansBoutons);
    };
    static afficherErreurConfirmation(message: string, delegueReponse: (sucess?: boolean) => void, type_messagebox?: etype_messagebox, sansBoutons?: boolean): void {
        ice2Dialog.afficherErreurConfirmation(message, delegueReponse, type_messagebox, sansBoutons);
    };
    ...
    static afficherMessageConfirmationPromise(message: string, afficherOuiNon: boolean, type_messagebox?: etype_messagebox/*, bloquerBtnValider: boolean = false, NomDeBoutonCustom?: { ok: string, cancel: string }*/, sansBoutons?: boolean): Promise<boolean> {
        return ice2Dialog.afficherMessageConfirmationPromise(message, afficherOuiNon, type_messagebox, sansBoutons);
    }
    ...
    static getUrlParameter(param: string): any {
        ...
        return iceOutils.getUrlParameterFromString(window.location.search.substring(1), param, true);
    };
    ...
    public static rechercheStringTous(valeursRecherche: string[], valeur2: string[]): boolean {
        ...
        return valeursRecherche.every(rech => { return iceOutils.rechercheString(rech, valeur2); });
    }

    public static rechercheStringUnParmiTous(valeursRecherche: string[], valeur2: string[]): boolean {

        return valeursRecherche.some(rech => { return iceOutils.rechercheString(rech, valeur2); });
    }
    ...
    public static copyToClipboard(str: string) {
        ...
        iceOutils.afficherMessageAlertifyLocaliseSuccess('Copié dans le presse papiers');
    }
    ...
    {
        if (iceOutils.ListKeyUpEventsCallback == null)
            iceOutils.ListKeyUpEventsCallback = [];

        let exist: boolean = iceOutils.ListKeyUpEventsCallback.some(s => s.IsKeyEvent(options.keyEvent));

        if (!exist)
        {
            let keyEvenement: eventKey = new eventKey(options.keyCode, options.keyEvent, options.callBack);
            iceOutils.ListKeyUpEventsCallback.push(keyEvenement);
    ...
    */
    ...
    {
        if (iceOutils.ListKeydownEventsCallback == null)
            iceOutils.ListKeydownEventsCallback = [];

        let exist: boolean = iceOutils.ListKeydownEventsCallback.some(s => s.IsKeyEvent(options.keyEvent));

        if (!exist)
        {
            let keyEvenement: eventKey = new eventKey(options.keyCode, options.keyEvent, options.callBack);
            iceOutils.ListKeydownEventsCallback.push(keyEvenement);
    ...
    public static removeKeyupEvent(keyCodeEvent:string): void 
    {
        if (iceOutils.ListKeyUpEventsCallback != null) 
        {
            let evenement = iceOutils.ListKeydownEventsCallback.filter(s => s.IsKeyEvent(keyCodeEvent));
    ...
    public static afficherMessageErreurLicence(nom_licence: string = null): void {
        ...
        let textAAfficher: string = new iceLString("La licence {0} nécessaire à l'affichage de ce contenu n'est pas présente sur votre version d'Zeus. Veuillez prendre contact avec votre référent.").format([nom_licence != null && nom_licence != "" ? "(" + nom_licence + ") " : ""]);
    ...
    public static afficherMessageErreurPage(info: string = null): void {
        ...
        let textAAfficher: string = new iceLString("Vous ne disposez pas des droits nécessaires à l'affichage de cette page.").format([info != null && info != "" ? "(" + info + ") " : ""]);
    ...
    public static Notification = {
        throwSimpleEvent: (source: number, evenement: string, clefEvenement: string | number) => {
            return iceOutils.notificationFullDom<number>(source, evenement, [{ data: Date.now(), clefEvenement: clefEvenement }]);
        },
        throwMultiSimpleEvent: (source: number, evenement: string, clefEvenement: (string | number)[]) => {

            return iceOutils.notificationFullDom<number>(source, evenement, clefEvenement.map(i => { return { data: Date.now(), clefEvenement: i } }));
        },

        listenSimpleEvent: (source: number, evenement: string, clefEvenement: string | number, onchange: (source: number) => void) => {
            return iceOutils.notificationListener<number>(source, evenement, clefEvenement, onchange);
        },
        listenOnlySimpleEvent: (evenement: string, clefEvenement: string | number, onchange: (source: number) => void) => {
            return iceOutils.notificationListenerPur<number>(evenement, clefEvenement, onchange);
        },
        createNewId: (): number => {
            return Math.random();
        }
    ...
    public static notificationListenerPur<T>(evenement: string, clefEvenement: string | number, onChange: (data: T) => void): () => void {
        return iceOutils.notificationListener(null, evenement, clefEvenement, onChange);
    }
    ...
    public static notificationFullDom<T>(source: number, evenement: string, liste: { data: T, clefEvenement: string | number }[]): void {
        ...
        return iceOutils.notificationFullDomByMessage(liste.map(i => {
    ...
    public static isMobile(): boolean {
        ...
        let isTestDeviceTypeMobile = iceOutils.getCssClassByDiviceType() != desktopDevice;
    ...
    public static IsNullOrEmpty(inStr: string): boolean {
        return iceOutils.isNullOrEmpty(inStr);
}
